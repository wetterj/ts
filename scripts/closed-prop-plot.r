...
pltFails <- ggplot(d) + theme(text=element_text(family="Times"), legend.justification=c(0,1), legend.position=c(0,1)) +
       ...
       geom_point(aes(x=base/1e6, y=upperBoundQR/1e6, shape="Upper bound qr", col="Upper bound qr"), size=4) + 
       geom_point(aes(x=base/1e6, y=upperBoundQ/1e6, shape="Upper bound q", col="Upper bound q"), size=4) + 
       geom_point(aes(x=base/1e6, y=upperBoundX/1e6, shape="Upper bound x", col="Upper bound x"), size=4) + 
       geom_point(aes(x=base/1e6, y=all/1e6, shape="All", col="All"), size=4) + 
...
d <- read.csv("./closed-time-data", sep=" ")
...
ggsave("./closed.pdf", arrangeGrob( pltFails, pltTime, ncol=2 ) ) 
